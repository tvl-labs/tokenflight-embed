...
TokenFlight widgets don't bundle a wallet — they talk to wallets through an adapter interface. Official adapters exist for Privy, AppKit, and Thirdweb, but you can build your own for any provider.
...
const swap = new TokenFlightSwap({
  container: '#swap',
  config: { theme: 'dark' },
  walletAdapter: new PrivyAdapter({ appId: 'your-app-id' }),
});
swap.initialize();
```
