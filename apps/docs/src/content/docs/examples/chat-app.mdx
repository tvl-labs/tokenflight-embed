...
The chat container uses a **fixed height** (e.g. `720px`) with `overflow-y: auto` on the message area so conversations scroll naturally. The widget renders inside a chat bubble — do **not** set a `min-width` on the bubble, as chat layouts need to adapt to narrow mobile screens. Let the widget fill the available bubble width instead.
